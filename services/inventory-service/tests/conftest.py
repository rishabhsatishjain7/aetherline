...
@pytest.fixture()
def _db_and_client():
    # File-based (not :memory:) so multiple connections/threads in the
    # concurrency test see the same data, exercising real locking behaviour
    # instead of each thread getting its own isolated in-memory DB.
...
@pytest.fixture()
def client(_db_and_client):
    c, _ = _db_and_client
    return c


@pytest.fixture()
def db_session(_db_and_client):
    """A raw session against the same test DB the API uses -- lets tests call
    consumer-side crud functions directly, simulating a Kafka message
    arriving, without needing a real Kafka broker in the test environment."""
    _, SessionLocal = _db_and_client
